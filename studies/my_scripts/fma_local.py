...
#collider = xt.Multiline.from_json('/eos/user/a/aradosla/SWAN_projects/Separation_adjust/collider_final_second_try.json')
...
x_phys = np.zeros((norm_intervals, num_particles))
y_phys = np.zeros((norm_intervals, num_particles))
zeta_phys = np.zeros((norm_intervals, num_particles))
px_phys = np.zeros((norm_intervals, num_particles))
py_phys = np.zeros((norm_intervals, num_particles))
pzeta_phys = np.zeros((norm_intervals, num_particles))
state_all = np.zeros((norm_intervals, num_particles), dtype=np.int32)
particles_id_all = np.empty((norm_intervals, num_particles), dtype=np.int32)
turns_totnorm = np.empty((norm_intervals, num_particles), dtype=np.int32)
...
if 1:
    ...
    for i in range(num_turns):
        ...
        # Store physical coordinates
            x_phys[interval_index, :] = particles_copy.x
            y_phys[interval_index, :] = particles_copy.y
            zeta_phys[interval_index, :] = particles_copy.zeta
            px_phys[interval_index, :] = particles_copy.px
            py_phys[interval_index, :] = particles_copy.py
            pzeta_phys[interval_index, :] = particles_copy.delta
            state_all[interval_index, :] = particles_copy.state
            turns_totnorm[interval_index, :] = np.ones(num_particles, dtype=np.int32) * (i + 1)
            particles_id_all[interval_index, :] = particles_copy.particle_id
    ...
    state_all = state_all.flatten()
    turns_totnorm = turns_totnorm.flatten()
    particles_id_all = particles_id_all.flatten()
    ...
    dffs.to_parquet(f'/eos/user/a/aradosla/SWAN_projects/FMA_studies_FLATTOP/FMA_1.6e11_2025_120cm_noise1time/fma_data.parquet')
    frev = 11245.5
    # %%
    dffs = pd.read_parquet(f'/eos/user/a/aradosla/SWAN_projects/FMA_studies_FLATTOP/FMA_1.6e11_2025_120cm_noise1time/fma_data.parquet')
    fig, ax = plt.subplots()
    print('Now plotting')
